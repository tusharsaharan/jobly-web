...
export function VideoGrid({
  ...
  // If LiveKit signaling server is offline or fails, seamlessly fall back to Local Device Mode
  if (error) {
    return (
      <LocalMediaCall
        userName={userName}
  ...
  return (
    <LiveKitRoom
      ...
      onError={() => {
        // Fall back to local device mode on connection drops
...
/** Fallback media call that uses browser-native camera & microphone when LiveKit server is offline */
  ...
  return (
    ...
    >
      ...
      <div className="mb-2 flex items-center justify-between gap-2 text-[11px]">
        <div className="flex items-center gap-1.5 rounded-full bg-emerald-500/15 px-2 py-0.5 font-semibold text-emerald-300">
          <span className="h-2 w-2 rounded-full bg-emerald-400 animate-pulse" />
          <span>Local Device Active</span>
        </div>

        {onRetryLiveKit && (
          <button
            onClick={onRetryLiveKit}
            title="Attempt connection to LiveKit server"
            className="flex items-center gap-1 text-[10px] text-[#888888] hover:text-[#7EE0C5] transition"
          >
            <RefreshCw className="h-3 w-3" />
            <span>Connect LiveKit</span>
          </button>
        )}
      </div>

      {/* Video Viewport */}
      <div className="min-h-0 flex-1 relative rounded-lg border border-[#2A2A2A] bg-[#0E0E0E] overflow-hidden flex items-center justify-center">
        {permissionError ? (
          <div className="flex flex-col items-center justify-center p-4 text-center">
            <AlertCircle className="mb-2 h-6 w-6 text-rose-400" />
            <p className="text-xs font-semibold text-rose-200">Device Access Required</p>
            <p className="mt-1 text-[11px] text-[#AAAAAA] max-w-xs">{permissionError}</p>
            <p className="mt-2 text-[10px] text-[#777777]">
              Please allow Camera and Microphone permissions in your browser address bar.
            </p>
          </div>
          ...
          <div className="relative w-full h-full">
            ...
            />
            <div className="absolute bottom-1.5 left-1.5 rounded bg-black/70 px-2 py-0.5 text-[10px] font-medium text-white">
              {userName} (You) {isScreenSharing ? "— Screen" : ""}
            </div>
          </div>
        ) : (
          <div className="flex flex-col items-center justify-center p-4 text-center">
            <VideoOff className="mb-2 h-6 w-6 text-[#777777]" />
            <p className="text-xs font-medium text-[#D4D4D4]">Camera is muted</p>
            <p className="mt-1 text-[11px] text-[#888888]">Click the camera icon below to turn video on.</p>
          </div>
        )}
      </div>
