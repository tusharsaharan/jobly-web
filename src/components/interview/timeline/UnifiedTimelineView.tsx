...
export interface TimelineItem {
  _id?: string;
  pipeline: "COMMUNICATION" | "CODING" | "WHITEBOARD" | "STAGE" | "AI" | "NOTE" | "SYSTEM";
  eventType: string;
  offsetMs: number;
...
export function UnifiedTimelineView({ events, onSelectEvent }: UnifiedTimelineViewProps) {
  ...
  return (
    <div className="flex h-full flex-col overflow-hidden rounded-xl border border-[#2A2A2A] bg-[#161616] p-2.5 text-xs text-white shadow-xl">
      {/* Header */}
      ...
      <div className="flex-1 space-y-1.5 overflow-y-auto pr-0.5">
        {filteredEvents.length === 0 ? (
          ...
          filteredEvents.map((ev, idx) => (
            ...
            >
              <span className="mt-0.5 rounded bg-[#111111] px-1 py-0.2 font-mono text-[9px] text-[#888888]">
              ...
              <div className="flex-1 min-w-0">
                <div className="flex items-center justify-between">
                  ...
                  <span className="text-[9px] text-[#666666] uppercase truncate">{ev.eventType.replace(".", " ")}</span>
                </div>

                <p className="mt-0.5 text-[11px] text-[#D0D0D0] leading-snug line-clamp-2">
                  {ev.payload?.text ||
                    (ev.payload?.stage ? `Stage: ${ev.payload.stage}` : null) ||
                    (ev.payload?.exitCode !== undefined
                      ? `Execution Exit ${ev.payload.exitCode} (${ev.payload.durationMs}ms)`
                      : null) ||
                    ev.eventType}
                </p>
              </div>
